...
from random_forest.evaluation import evaluate
from random_forest.training.remove_correlated_features import remove_correlated_features
from random_forest.analysis.visualize_random_forest import make_plots
from random_forest.utils import do_scaling, get_metadata_columns, _internal_validation
...
def _make_train_test_split(full_dataframe: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
...
def main(do_split: bool = False):
    ...
    if do_split:
        train_dataframe, test_dataframe = _make_train_test_split(full_dataframe)
